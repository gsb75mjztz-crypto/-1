...
- **VERIFY:** every text/background colour pairing in Section 2 against WCAG AA contrast thresholds (4.5:1 normal text, 3:1 large text/UI components) using an automated tool (e.g. axe-core, Stark) before merge. The palette in this document is designed with AA compliance as the intent, but exact contrast ratios have not been computationally verified in this document and must not be assumed correct without that check — this is the one place in this spec where "designed to pass" and "confirmed to pass" are different claims, and only the second one clears the bar.
  - **Update, Milestone 2 audit:** `--color-text-muted` was computed (WCAG relative-luminance formula) and failed — 2.78:1 on `--color-bg`, 2.95:1 on `--color-surface`, both below even the 3:1 large-text floor. Corrected to `#716D63` (4.86:1 / 5.16:1). `--color-text-secondary`, `--color-accent` on `--color-accent-tint`, and `--color-accent-secondary` on `--color-surface` were also computed and pass.
  - **Update, Milestone 3:** the confidence badge colours are now actually rendered (ETF pages) and were computed: high `#3B6D11` on `#EAF3DE` = 5.43:1, medium `#854F0B` on `#FAEEDA` = 5.87:1, low `#791F1F` on `#FCEBEB` = 8.98:1 — all pass. `--color-positive` (used for the Data Quality checkmarks) on both `--color-surface` and `--color-bg` also passes (6.21:1 / 5.85:1). `--color-negative` remains unverified — still unused anywhere in the shipped product as of this milestone (no error states have been built yet that use it for text).
  - **Update, Milestone 4:** the Comparison Tool introduced two new pairings — the fund search dropdown's active option and the selected-fund chip both render text on `--color-accent-tint`. Computed: `--color-text-primary` (`#211F1C`) on `--color-accent-tint` (`#E1F5EE`) = 14.48:1, `--color-text-secondary` (`#6B6862`) on the same background = 4.89:1 — both pass. `--color-negative` remains unused/unverified.
  - **Update, post-Milestone-4 optimisation pass:** `--color-negative` (`#A32D2D`) is now used for the first time — the Comparison Tool's fetch-error text. Computed: 6.66:1 on `--color-bg`, 7.07:1 on `--color-surface` — both pass.
